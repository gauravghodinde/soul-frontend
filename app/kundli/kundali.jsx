...
export default function Kundali() {
    ...
    function convertLatitude(lat) {
        const latDirection = lat >= 0 ? 'N' : 'S';  // Determine North/South direction
        const latZone = Math.abs(Math.floor(lat));  // Use the absolute value of the latitude
        return `${latDirection}${latZone}`;
    }
    function convertLongitude(lon) {
        const lonDirection = lon >= 0 ? 'E' : 'W';  // Determine East/West direction
        const lonZone = Math.abs(Math.floor(lon));  // Use the absolute value of the longitude
        return `${lonZone}${lonDirection}`;
    }
    ...
    const handlekundali = async (e) => {
        e.preventDefault();
        const name = "gaurav";
        const date = responseData.formdata.dob;
        const time = responseData.formdata.tob;

        const lat = convertLatitude(responseData.current_transit.coordinates.latitude);
        const lon = convertLongitude(responseData.current_transit.coordinates.longitude);
        try {
